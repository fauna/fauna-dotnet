...
internal class NullableDocumentDeserializer<T> : BaseDeserializer<NullableDocument<T>> where T : class
{
    ...
    public override NullableDocument<T> Deserialize(MappingContext context, ref Utf8FaunaReader reader)
    {
        ...
        if (typeof(T) == typeof(DocumentRef)) return (_nullabelDocRef.Deserialize(context, ref reader) as NullableDocument<T>)!;
        if (typeof(T) == typeof(NamedDocumentRef)) return (_nullabelNamedDocRef.Deserialize(context, ref reader) as NullableDocument<T>)!;
