...
public class ClientTests
{
    ...
    public async Task CreateClient()
    {
        ...
        var c = new Client(new ClientConfig("secret") { Endpoint = Constants.Endpoints.Local });
        var r = await c.QueryAsync<string>("let x = 123; x", queryTags: new Dictionary<string, string> { { "foo", "bar" }, {"baz", "luhrmann"} });
        Write(r.Data);
